...
describe("commonStore Actions", () => {
  ...
  describe("launchAuthentif", () => {
    ...
    it("Should not commit (user not authenticated yet)", async done => {
      securityManager.getUser.mockImplementation(() => undefined)
      TestsUtils.executeAction({
        action,
        done
      })
      expect(await securityManager.signIn).toHaveBeenCalledTimes(1)
    })
  })
